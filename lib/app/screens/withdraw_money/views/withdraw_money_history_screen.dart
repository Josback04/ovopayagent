...
class WithdrawMoneyHistoryScreen extends StatefulWidget {
  const WithdrawMoneyHistoryScreen({super.key});

  @override
  State<WithdrawMoneyHistoryScreen> createState() => _WithdrawMoneyHistoryScreenState();
}

class _WithdrawMoneyHistoryScreenState extends State<WithdrawMoneyHistoryScreen> {
  final ScrollController historyScrollController = ScrollController();
  void fetchData() {
    Get.find<WithdrawMoneyController>().getDonationHistoryDataList(
      forceLoad: false,
    );
  }
    ...
    return GetBuilder<WithdrawMoneyController>(
      builder: (controller) {
        return MyCustomScaffold(
          pageTitle: MyStrings.withdrawHistory,
          body: CustomAppCard(
            child: Column(
              ...
              children: [
                Flexible(
                  ...
                  child: controller.isHistoryLoading
                      ...
                      : controller.withdrawHistoryList.isEmpty
                          ? NoDataWidget(
                              text: MyStrings.noWithdrawHistoryToShow.tr,
                            )
                          : RefreshIndicator(
                              ...
                              child: ListView.builder(
                                ...
                                itemCount: controller.withdrawHistoryList.length + 1,
                                itemBuilder: (context, index) {
                                  if (controller.withdrawHistoryList.length == index) {
                                    return controller.hasNext() ? const CustomLoader(isPagination: true) : const SizedBox();
                                  }
                                  var item = controller.withdrawHistoryList[index];
                                  bool isLastIndex = index == controller.withdrawHistoryList.length - 1;
                                  return CustomCompanyListTileCard(
                                    leading: Stack(
                                      children: [
                                        MyNetworkImageWidget(
                                          ...
                                          imageAlt: "${item.method?.name}",
                                          isProfile: true,
                                          imageUrl: item.method?.getImageUrl() ?? "",
                                        ),
                                    ...
                                    imagePath: item.method?.getImageUrl(),
                                    title: item.method?.name ?? "",
                                    subtitle: MyStrings.withdrawMessage.tr.rKv({
  ...
  Widget buildDetailsSectionSheet({
    required WithdrawalDataModel item,
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
              ...
              Expanded(
                child: CardColumn(
                  ...
                  body: MyUtils.getUserAmount(
                    (AppConverter.formatNumberDouble(item.amount ?? "") - AppConverter.formatNumberDouble(item.charge ?? "")).toString(),
                  ),
                  subBody: "${item.method?.currency} ${MyUtils.getUserAmount(item.finalAmount ?? "", showOnlyAmount: true)}",
                  space: 5,
                  crossAxisAlignment: CrossAxisAlignment.start,
                ),
              ),
          ...
          spaceDown(Dimensions.space10),
          if (item.adminFeedback?.isEmptyString == false) ...[
            Row(
              children: [
                Expanded(
                  child: CardColumn(
